...
curpos=($(echo "show slave status\G" | ssh -q $1 mysql -Ar | egrep " Master_Host| Master_Log_File| Exec_Master_Log_Pos| Relay_Log_File| Relay_Log_Pos" | cut -d ":"  -f 2))
master=${curpos[0]}
...
binlog_base=$(echo "select @@log_bin_basename" | ssh -q $master mysql -ANr)
relay_base=$(echo "select @@datadir" | ssh -q $master mysql -ANr)
if [ "$3" = "dead" ]
then
    echo -n "looking up Xid in relay log for position $exec_pos... "
    xid=$(ssh -q $1 sudo mysqlbinlog $relay_base/$relay | fgrep "end_log_pos $exec_pos " | cut -d"=" -f 2 | tr -d " ")
else
    echo -n "looking up Xid in $binlog for position $exec_pos... "
    xid=$(ssh -q $master sudo mysqlbinlog $(dirname $binlog_base)/$binlog | fgrep "end_log_pos $exec_pos " | cut -d"=" -f 2 | tr -d " ")
fi
echo $xid
binlog_base=$(echo "select @@log_bin_basename" | ssh -q $new_master mysql -ANr)
echo -n "searching binlogs on $2 using Xid range search... "
for b in $(ssh -q $new_master ls -t $binlog_base*.[0-9]*)
do
    new_pos_binlog=$(basename $b)
    echo -n $new_pos_binlog" "
    bxid=$(ssh -q $new_master sudo mysqlbinlog $b | fgrep  -m 1 "Xid = " | cut -d "=" -f 2 | tr -d " ")
    [ $bxid -lt $xid ] && break
done
echo
echo -n "looking up position in $new_pos_binlog for Xid $xid... "
new_pos=$(ssh -q $new_master sudo mysqlbinlog $b | fgrep "Xid = $xid" | tr -s " " | cut -d " " -f 7)
echo $new_pos
