...
d60=$(echo "select id from growth where sample_date = date_sub(curdate(), interval 2 month) limit 1" | mysql -u$USER -p$PASS -ANr tablegrowth | wc -l)
echo
case $type in
...
(
	...
	then
		case $type in
			1) echo -n "	t60.table_size";;
			2) echo -n "	concat(truncate(100 - (t60.table_size * 100 / t0.table_size),1), '%')";;
			3) echo -n "	t0.table_size - t60.table_size";;
		esac
	else
		echo -n "	'N/A'"
	fi
	...
	echo "from growth t0 "
	echo "  left join growth t1 on t1.server = t0.server and t1.schema_name = t0.schema_name and t1.table_name = t0.table_name and t1.sample_date =  date_sub(curdate(), interval 1 day)"
        [ $d7 -eq 1 ] && echo " left join growth t7 on t7.server = t0.server and t7.schema_name = t0.schema_name and t7.table_name = t0.table_name and t7.sample_date =  date_sub(curdate(), interval 1 week)"
        [ $d14 -eq 1 ] && echo "        left join growth t14 on t14.server = t0.server and t14.schema_name = t0.schema_name and t14.table_name = t0.table_name and t14.sample_date =  date_sub(curdate(), interval 2 week)"
        [ $d30 -eq 1 ] && echo "        left join growth t30 on t30.server = t0.server and t30.schema_name = t0.schema_name and t30.table_name = t0.table_name and t30.sample_date =  date_sub(curdate(), interval 1 month)"
        [ $d60 -eq 1 ] && echo "        left join growth t60 on t60.server = t0.server and t60.schema_name = t0.schema_name and t60.table_name = t0.table_name and t60.sample_date =  date_sub(curdate(), interval 2 month)"
        echo "where t0.sample_date = curdate()"
	case $type in 
